...
if(@ARGV>0) { # check for another argument
    ...
    if($oflag =~ m/^(s|p|e|t|d|a|d1|d2|d3|d4|d5)$/i) { #hj
    } else {
...
sub readDOSi{
    ...
    if($cols == 19) {
        if($oflag =~ /^s$/i) { $colup = 1; $coldown = 2; }
        if($oflag =~ /^p$/i) { $colup = 3; $coldown = 4; }
        if($oflag =~ /^d1$/i) { $colup = 11; $orbital = 1; } #hj
        if($oflag =~ /^d2$/i) { $colup = 12; $orbital = 1; } #hj
        if($oflag =~ /^d3$/i) { $colup = 13; $orbital = 1; } #hj
        if($oflag =~ /^d4$/i) { $colup = 14; $orbital = 1; } #hj
        if($oflag =~ /^d5$/i) { $colup = 15; $orbital = 1; } #hj
        if($oflag =~ /^e$/i) { $colup = 5; $coldown = 6; $orbital = 2; } #hj
        if($oflag =~ /^t$/i) { $colup = 7; $coldown = 8; $orbital = 3; } #hj
        if($oflag =~ /^d$/i) { $colup = 9; $coldown = 10; $orbital = 5; } #hj
        ...
        while ($in = <DOS>) {
            ...
            if($cols == 19) {
                if($colup == 1) {
                    ...
                    $dosvup = $line[3]+$line[5]+$line[7]; #hj
                } elsif($colup == 5) {
                    $dosval = $line[13]-$line[14]+$line[17]-$line[18]; #hj
                    $dosvup = $line[13]+$line[17]; #hj
                } elsif($colup == 7) {
                    $dosval = $line[9]-$line[10]+$line[11]-$line[12]+$line[15]-$line[16]; #hj
                    $dosvup = $line[9]+$line[11]+$line[15]; #hj
                } elsif($colup == 9) {
                    $dosval = $line[9]-$line[10]+$line[11]-$line[12]+$line[13]-$line[14]+$line[15]-$line[16]+$line[17]-$line[18];
                    $dosvup = $line[9]+$line[11]+$line[13]+$line[15]+$line[17]; #hj
                } elsif($colup == 11) {
                    $dosval = $line[9]-$line[10];
                    $dosvup = $line[9]; #hj
                } elsif($colup == 12) {
                    $dosval = $line[11]-$line[12];
                    $dosvup = $line[11]; #hj
                } elsif($colup == 13) {
                    $dosval = $line[13]-$line[14];
                    $dosvup = $line[13]; #hj
                } elsif($colup == 14) {
                    $dosval = $line[15]-$line[16];
                    $dosvup = $line[15]; #hj
                } elsif($colup == 15) {
                    $dosval = $line[17]-$line[18];
                    $dosvup = $line[17]; #hj
                } else {
                    ...
                    $dosvup = $line[1]+$line[3]+$line[5]+$line[7]+$line[9]+$line[11]+$line[13]+$line[15]+$line[17]; #hj
                }
            }
...
#print "$Emin,$Emax,$numenemin,$numenemax,$numene\n";
# Calculate the average energy state
$dossum = $dstsum = $occsum = $ocusum = $unusum = 0; #hj
if($w) {
...
if($orbital > 0){
    $dosocc = $occsum/$dossum*$orbital; #hj
    $douocc = $ocusum/$dossum*$orbital; #hj
    $dodocc = $dosocc - $douocc; } #hj
...
#print "\n";
#print "total# $dossum\n"; #total 
#print "occupied# $occsum\n"; #occupied
print "eg occupancy: $dosocc\n";
#print "occupied-up# $ocusum\n"; #occupied-up 
#print "$unusum\n"; #unoccupied-up
print "center: $eneavg\n"; #band-center
#if($w) {
#    print "\n";
