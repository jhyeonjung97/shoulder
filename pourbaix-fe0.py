...
df['comp'] = 'FeC' + df.index.str.upper().str.replace("-", "")
df['comp'] = df['comp'].str.replace('FeCVAC', 'FeC')
df['name'] = df['name'].str.replace('FeNC(VAC)', 'Fe⁺²+H₂NC', regex=False)
df['comp'] = df['comp'].str.replace('FeCCLEAN', 'FeC')
df['name'] = df['name'].str.replace('FeNC(CLEAN)', 'FeNC(clean)')
df['comp'] = df['comp'].str.replace('FeCMH', 'FeCH')
df['comp'] = df['comp'].str.replace('FeCNH', 'FeCH')

df['name'] = 'FeNC(' + df.index.str.upper() + ')'
df['comp'] = 'FeN' + df.index.str.upper().str.replace("-", "")
df['comp'] = df['comp'].str.replace('FeNVAC', 'FeN')
df['name'] = df['name'].str.replace('FeNC(VAC)', 'Fe⁺²+H₂NC', regex=False)
df['comp'] = df['comp'].str.replace('FeNCLEAN', 'FeN')
df['name'] = df['name'].str.replace('FeNC(CLEAN)', 'FeNC(clean)')
df['comp'] = df['comp'].str.replace('FeNMH', 'FeNH')
df['comp'] = df['comp'].str.replace('FeNNH', 'FeNH')

# df['name'] = 'FeNC(' + df.index.str.upper() + ')'
# df['comp'] = 'FeX' + df.index.str.upper().str.replace("-", "")
# df['comp'] = df['comp'].str.replace('FeXVAC', 'FeX')
# df['name'] = df['name'].str.replace('FeNC(VAC)', 'Fe⁺²+H₂NC', regex=False)
...
df['energy'] = df['G'] - df.loc['vac', 'dG'] + charge * potential - water * (df['#O'] + df['#OH'] + df['#OOH']*2)
